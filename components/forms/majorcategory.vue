<template>
  <div class="form-main h-full bg-gray-300 rounded-lg overflow-hidden">
      ...
      <h1 class="text-gary-700 text-3xl text-center my-5">Create </h1>
          <form class="text-gray-600" @submit.prevent="create">
            <div class="flex flex-col max my-3">
              <ValidationProvider name="Major category name" rules="required|min:3" v-slot="{ errors }">
              <input type="text" placeholder="Major category name" v-model="form.name" class="placeholder-black border rounded w-full py-2 px-4 outline-none focus:shadow-inner"/>
